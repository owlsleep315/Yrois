...
- 환승 등록: 한 열차를 클릭한 상태에서 드래그로 다른 열차를 클릭하면 환승을 등록할 수 있다. 환승하는 열차는 화살표로 표시된다. 자기 자신으로 드래그 하는것으로 화살표를 지울 수 있다. 
- 알림 기능: 승하차보조건이 10분 남은 경우 알림이 팝업으로 뜬다. 닫기 버튼을 누르는 것으로 창을 지울 수 있다.
- 타임라인으로 표시(필수x): 간트 차트처럼 각 담당자들의 할일 타임라인을 표시하여 할일을 잘 파악하도록 도와준다. 타임라인 버튼을 누르면 리스트 대신 가로로 쭉 이어진 타임라인으로 표시하게 된다. 각 열차시간기준 전후 10분을 차지하는 블록으로 구성되며 색은 분류에 따라 결정된다.